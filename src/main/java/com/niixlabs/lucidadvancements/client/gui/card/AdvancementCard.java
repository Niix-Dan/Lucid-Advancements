...
public final class AdvancementCard implements Comparable<AdvancementCard> {
    ...
    private void populateCriteria(Identifier advancementId, AdvancementProgress progress) {
    ...
    public void renderBackgroundAndText(GuiGraphicsExtractor guiGraphics, Font font, int x, int y, int width, int mouseX, int mouseY,
        ...
        if (hidden) {
            ...
            guiGraphics.centeredText(font, "?", x + 20, iconY + 4, LucidConfig.cardHiddenMarkColor);
        }

        guiGraphics.text(font, title, x + LucidConfig.cardTextXOffset, y + 8, palette.title(), true);

        int descriptionY = y + 22;
        for (FormattedCharSequence line : wrappedDescription) {
            guiGraphics.text(font, line, x + LucidConfig.cardTextXOffset, descriptionY, LucidConfig.cardDescriptionColor, true);
            descriptionY += LucidConfig.cardLineHeight;
        }

        guiGraphics.text(font, done ? LucidConfig.cardIconDone : LucidConfig.cardIconLocked, x + width - LucidConfig.cardStatusIconXOffset,
                y + (baseHeight / 2) - 4, palette.status(), true);

        if (!hidden) {
            guiGraphics.text(font, LucidConfig.cardIconTracked, x + width - LucidConfig.cardTrackIconXOffset,
    ...
    private void renderCriteriaSection(GuiGraphicsExtractor guiGraphics, Font font, int x, int y, int width) {
        guiGraphics.fill(x + 12, y + baseHeight - 5, x + width - 12, y + baseHeight - 4, LucidConfig.cardDividerColor);

        int lineY = y + baseHeight + 3;
        guiGraphics.text(font, Component.translatable(Constants.MOD_ID + ".gui.card.requirements"),
                x + LucidConfig.cardTextXOffset, lineY, LucidConfig.cardRequirementsLabelColor, true);
        lineY += 11;

        if (criteria.isEmpty()) {
            guiGraphics.text(font, Component.translatable(Constants.MOD_ID + ".gui.card.no_requirements"),
        ...
        for (CriterionEntry entry : criteria) {
            ...
            guiGraphics.text(font, entry.formattedLabel(), x + 48, lineY, color, true);
    ...
    public void renderIcon(GuiGraphicsExtractor guiGraphics, int x, int y) {
        ...
        guiGraphics.pose().pushMatrix();
        guiGraphics.pose().translate(0, 0);
        guiGraphics.item(display.getIcon().create(), x + LucidConfig.cardIconXOffset, iconY);
        guiGraphics.pose().popMatrix();
    }
    ...
    public ItemStack getHoveredIcon(int mouseX, int mouseY, int x, int y, int viewportY, int viewportHeight, boolean isBlocked) {
        ...
        return (withinIcon && withinViewport) ? display.getIcon().create() : null;
    }
