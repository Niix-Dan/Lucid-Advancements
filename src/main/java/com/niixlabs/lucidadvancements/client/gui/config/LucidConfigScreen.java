...
public class LucidConfigScreen extends Screen {
    ...
    private void renderTopBar(GuiGraphicsExtractor guiGraphics, int scaledMouseX, int scaledMouseY) {
        ...
        guiGraphics.text(font, Component.translatable(Constants.MOD_ID + ".gui.config.title"), sidebarWidth + LucidConfig.screenContentMargin, 20, LucidConfig.screenHeaderTitleColor, true);
        ...
        guiGraphics.centeredText(font, Component.translatable(Constants.MOD_ID + ".gui.config.save_and_exit"), backX + BACK_BUTTON_WIDTH / 2, BACK_BUTTON_Y + 5, hovered ? LucidConfig.widgetTextHovered : LucidConfig.widgetTextIdle);
    }
    ...
    private void renderSidebar(GuiGraphicsExtractor guiGraphics, double scaleFactor, int scaledMouseX, int scaledMouseY) {
        ...
        for (SidebarSection section : sidebarSections) {
            ...
            guiGraphics.pose().translate(10, textY);
            ...
            guiGraphics.text(font, displayTitle, 0, 0, selected ? LucidConfig.screenSidebarTextSelected : LucidConfig.screenSidebarTextIdle, true);
            guiGraphics.pose().popMatrix();
    ...
    private void renderContent(GuiGraphicsExtractor guiGraphics, double scaleFactor, int scaledMouseX, int scaledMouseY, float partialTick) {
    ...
    public boolean mouseDragged(MouseButtonEvent event, double dragX, double dragY) {
        ...
        for (ConfigEntry entry : entries) {
            if (entry.mouseDragged(event, dragX * scaleFactor, dragY * scaleFactor)) {
                return true;
            }
        }
    ...
    public static abstract class ConfigEntry {
        ...
        public abstract void render(GuiGraphicsExtractor guiGraphics, Font font, int x, int y, int width, int mouseX, int mouseY, float partialTick);
        ...
        public void renderLabel(GuiGraphicsExtractor guiGraphics, Font font) {
            guiGraphics.text(font, name, x + 10, y + 6, LucidConfig.widgetTextIdle, true);
            if (!comment.isEmpty()) {
                guiGraphics.pose().pushMatrix();
                guiGraphics.pose().scale(0.8f, 0.8f);
                guiGraphics.text(font, comment, (int) ((x + 10) / 0.8f), (int) ((y + 18) / 0.8f), 0xFFAAAAAA, true);
                guiGraphics.pose().popMatrix();
            }
        }
    ...
    public static class SectionHeaderEntry extends ConfigEntry {
        ...
        @Override
        public void render(GuiGraphicsExtractor guiGraphics, Font font, int x, int y, int width, int mouseX, int mouseY, float partialTick) {
            guiGraphics.fill(x, y + 20, x + width, y + 35, LucidConfig.screenHeaderDividerColor);
            guiGraphics.pose().pushMatrix();
            guiGraphics.pose().scale(1.2f, 1.2f);
            guiGraphics.text(font, title, (int) ((x + 5) / 1.2f), (int) ((y + 24) / 1.2f), LucidConfig.screenHeaderTitleColor, true);
            guiGraphics.pose().popMatrix();
        }
    ...
    public static class TextOptionEntry extends ConfigEntry {
        ...
        @Override
        public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
            boolean clicked = editBox.mouseClicked(event, doubleClick);
            if (clicked) {
        ...
        @Override
        public boolean mouseDragged(MouseButtonEvent event, double dragX, double dragY) {
            return editBox.mouseDragged(event, dragX, dragY);
        }
