...
final class LucidDropdown<T> extends AbstractWidget {
    ...
    protected void extractWidgetRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        guiGraphics.text(font, labelProvider.apply(selected), x1 + 6, y1 + (height - 8) / 2, textColor);
        guiGraphics.text(font, open ? LucidConfig.dropdownArrowOpen : LucidConfig.dropdownArrowClosed, x2 - 14, y1 + (height - 8) / 2, textColor);
    }

    void renderOptions(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY) {
        if (!open) {
            return;
        }
        ...
        for (T option : options) {
            ...
            guiGraphics.text(font, labelProvider.apply(option), x1 + 6, optY + (LucidConfig.dropdownOptionHeight - 8) / 2, textColor);
