...
public final class LucidAdvancementsOverlay {
    ...
    public static void render(GuiGraphicsExtractor guiGraphics, float partialTick) {
        ...
        if (scaleMod != 1.0f) {
            guiGraphics.pose().scale(scaleMod, scaleMod);
        }
    ...
    private static CachedOverlayBox buildOverlayBox(Font font, Identifier loc, DisplayInfo display, List<String> rawRemaining) {
    ...
    private static void renderBox(GuiGraphicsExtractor guiGraphics, Font font, DisplayInfo display, CachedOverlayBox cachedBox, List<String> rawRemaining, int startX, int startY) {
        ...
        ItemStack iconStack = display.getIcon().create();
        guiGraphics.pose().pushMatrix();
        guiGraphics.item(iconStack, startX + LucidConfig.overlayIconXOffset, startY + LucidConfig.overlayIconYOffset);
        guiGraphics.pose().popMatrix();

        guiGraphics.text(font, cachedBox.titleText(), startX + LucidConfig.overlayTitleXOffset, startY + LucidConfig.overlayTitleYOffset, cachedBox.titleColor(), true);

        int textY = startY + LucidConfig.overlayCriteriaStartYOffset;
        for (String line : cachedBox.critLines()) {
            guiGraphics.text(font, line, startX + LucidConfig.overlayCriterionXOffset, textY, LucidConfig.overlayCriterionTextColor, true);
        ...
        if (rawRemaining.size() > LucidConfig.overlayMaxVisibleCriteria) {
            ...
            guiGraphics.text(font, Component.translatable(Constants.MOD_ID + ".overlay.hidden_count", hiddenCount), startX + LucidConfig.overlayCriterionXOffset, textY, LucidConfig.overlayHiddenCountTextColor, true);
        }
    }
}